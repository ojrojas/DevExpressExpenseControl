...
public class RequestProvider(HttpMessageHandler _messageHandler) : IRequestProvider
{
    ...
    private static async Task<T?> ReadFromJsonAsync<T>(HttpContent content)
    {
        ...
        var data = await JsonSerializer.DeserializeAsync(contentStream, typeof(T)).ConfigureAwait(false);
        return (T?)data;
    }

    private static JsonContent SerializeToJson<T>(T data)
    {
        var typeInfo = typeof(T).GetType() ?? throw new InvalidOperationException($"Missing type info for {typeof(T)}");
